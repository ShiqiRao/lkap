...
import { ParsedLink, LinkReference, GlobalLinkReferences } from '../types';
...
export class LinkParser {
  ...
  /**
   * Parse all links from a document using global link references
   * @param document The document to parse
   * @param globalReferences Global link references from all documents
   * @returns Array of parsed links
   */
  static parseDocumentLinksWithReferences(document: vscode.TextDocument, globalReferences: GlobalLinkReferences): ParsedLink[] {
    const text = document.getText();
    const links: ParsedLink[] = [];

    // Parse wiki-style links using global references
   ...
   * @param linkReferences Link references from metadata
   * @returns Array of parsed wiki links
   */
  private static parseWikiLinks(text: string, document: vscode.TextDocument, linkReferences: GlobalLinkReferences): ParsedLink[] {
    const links: ParsedLink[] = [];
    let match: RegExpExecArray | null;
   ...
   */
  private static resolveWikiLinkTarget(linkText: string, sourcePath: string, linkReferences: GlobalLinkReferences): string {
    try {
      // First, check if there's a link reference for this wiki link
      const linkRef = linkReferences.getBestMatch(linkText, sourcePath);
      if (linkRef) {
        // Use the path from the link reference, relative to the reference's source file
        ...
        // Relative to the reference's source file directory
        const referenceDir = path.dirname(linkRef.sourceFile);
        return path.resolve(referenceDir, decodedPath);
      }
  ...
  /**
   * Extract link references from a document (public method)
   * @param document The document to extract references from
   * @returns Map of reference names to link references
   */
  static extractLinkReferencesFromDocument(document: vscode.TextDocument): Map<string, LinkReference> {
    const text = document.getText();
    return this.extractLinkReferences(text);
  }

  /**
   * Find all links in a workspace with global link references
   * @param globalReferences Global link references
   * @returns Promise resolving to an array of parsed links
   */
  static async findAllLinksWithReferences(globalReferences: GlobalLinkReferences): Promise<ParsedLink[]> {
    ...
    for (const fileUri of mdFiles) {
      try {
        ...
        const documentLinks = this.parseDocumentLinksWithReferences(document, globalReferences);
  ...
  /**
   * Find all wiki links in a workspace (legacy method for backward compatibility)
   * @returns Promise resolving to an array of parsed links
   */
